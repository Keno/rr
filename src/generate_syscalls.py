...
def write_syscallname_arch(f):
    ...
    f.write("\n");
    for specializer, arch in [("X86Arch", "x86"), ("X64Arch", "x64"), ("AA64Arch", "generic")]:
...
is_${syscall}_syscall(int syscallno, SupportedArch arch) {
  switch (arch) {
    ...
    case aarch64:
      return syscallno >= 0 && syscallno == AA64Arch::${syscall};
    default:
      DEBUG_ASSERT(0 && "unsupported architecture");
      return false;
  }
...
generators_for = {
    ...
    'SyscallEnumsAA64': lambda f: write_syscall_enum(f, 'generic'),
    ...
    'SyscallEnumsForTestsAA64': lambda f: write_syscall_enum_for_tests(f, 'generic'),
