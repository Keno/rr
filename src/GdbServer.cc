...
static remote_ptr<void> base_addr_from_rendezvous(Task* t, string fname)
{
  ...
  remote_ptr<NativeArch::link_map> link_map = t->read_mem(REMOTE_PTR_FIELD(r_debug_remote, r_map));
  while (link_map != nullptr) {
    if (fname == t->read_c_str(t->read_mem(REMOTE_PTR_FIELD(link_map, l_name)))) {
      return t->read_mem(REMOTE_PTR_FIELD(link_map, l_addr));
    }
    link_map = t->read_mem(REMOTE_PTR_FIELD(link_map, l_next));
  }
  return nullptr;
}
